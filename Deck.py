...
class Deck:
    ...
    def drawSix(self):
        ...
        drawn = np.random.choice(deckExpanded, 6, replace=False)
        for card in drawn:
            self.deck[card] -= 1
    ...
    def exchange(self, remove, add):
        #Removes a card with idx remove from the player's hand, adds one with idx add in its place
        self.hand[remove] -= 1
        self.hand[add] += 1